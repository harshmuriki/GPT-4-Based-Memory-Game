...
def random_array(num_grid, radius, prompt):
    array, images_dict = game_backend.backend(num_grid, radius, prompt)
    # print("arr", array)
    return array, images_dict


def iteration(prompt, num_grid=6):
    ...
    width = 40
    array, images_dict = random_array(num_grid, circle_radius, prompt)
    ...
    player1_surface = player1.render('0', True, "black")
    global player1_rect
...
def set_circles_to_default(positions, screen, circle_radius):
    ...
    for i in range(positions.shape[0]):
        for j in range(positions.shape[1]):
            ...
            pygame.draw.rect(
                screen, "white", (x - width/2, y-width/2, width, width))
...
def set_circle_right(positions, selected_circles):
    for image, circle_center_idx in selected_circles:
        positions[circle_center_idx[0], circle_center_idx[1]][1] = "black"
...
def set_player_score(screen, player, score, turn):
    score = str(score)
    player_surface = player.render(score, True, "black")

    if turn == 1:
        pygame.draw.rect(screen, "pink", player1_rect)
        return player_surface, None
    else:
        pygame.draw.rect(screen, "pink", player2_rect)
